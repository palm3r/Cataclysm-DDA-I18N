...
void mattack::tentacle(game *g, monster *z)
{
 ...
 g->add_msg(_("Your %s is hit for %d damage!"), body_part_name(hit, side).c_str(),
            dam);
...
// Throw monsters
   ...
   if (g->u.posx == x && g->u.posy == y) { // Throw... the player?! D:
    ...
    for (int i = 0; i < traj.size() && !hit_wall; i++) {
     ...
     if (i > 0 && monhit != -1 && !g->z[monhit].has_flag(MF_DIGS)) {
      if (g->u_see(traj[i].x, traj[i].y, t))
       g->add_msg(_("You hit a %s!"), g->z[monhit].name().c_str());
      if (g->z[monhit].hurt(damage))
...
void mattack::bite(game *g, monster *z)
{
 ...
 g->add_msg(_("Your %s is bitten for %d damage!"), body_part_name(hit, side).c_str(),
            dam);
 g->u.hit(g, hit, side, dam, 0);
 if(one_in(10)){
