...
int player::hit_mon(game *g, monster *z, bool allow_grab) // defaults to true
{
 ...
 std::string verb = _("hit");
 std::string target = i18n::format(_("the %s"), z->name().c_str());
...
// Divide their dodge roll by 2 if this is a grab
 ...
 std::string target = (p.is_npc() ? i18n::format(_("%s's "), p.name.c_str()) : _("your "));
...
// Move our weapon to a temp slot, if it's not unarmed
  if (p.weapon.has_technique(TEC_BREAK, &p) &&
   ...
   if (is_u)
    g->add_msg(P_("%s break the grab!", "%s breaks the grab!", p.is_npc()), target.c_str());
...
{
 bool mon = (z != NULL);
 std::string You = (is_npc() ? name : _("You"));
 std::string target = (mon ? i18n::format(_("the %s"), z->name().c_str()) :
                       (p->is_npc() ? p->name : _("you")));
...
// We knock them back from a tile adjacent to us!
 ...
 case TEC_WIDE: {
   ...
   for (int y = posy - 1; y <= posy + 1; y++) {
    if (x != tarx || y != tary) { // Don't double-hit our target
     ...
     if (mondex != -1 && hit_roll() >= rng(0, 5) + g->z[mondex].dodge_roll()) {
      ...
      g->z[mondex].hurt(dam);
      if (u_see)
       g->add_msg(P_("%s hit %s for %d damage!", "%s hits %s for %d damage!", is_npc()), You.c_str(),
                                                target.c_str(), dam);
     }
     int npcdex = g->npc_at(x, y);
     if (npcdex != -1 &&
  ...
  if (!is_npc())
   g->add_msg(_("%d enemies hit!"), count_hit);
 } break;

 case TEC_DISARM:
  g->m.add_item(p->posx, p->posy, p->remove_weapon());
  if (u_see)
   g->add_msg(P_("%s disarm %s!", "%s disarms %s!", is_npc()), You.c_str(), target.c_str());
  break;
...

{
 ...
 bool mon = (z != NULL);
 std::string You = (is_npc() ? name : _("You"));
 std::string your = (is_npc() ? (male ? _("his") : _("her")) : _("your"));
 std::string target = (mon ? i18n::format(_("the %s"), z->name().c_str()) : p->name);
 bool u_see = (!is_npc() || g->u_see(posx, posy, junk));
 ...
 switch (technique) {
  ...
  case TEC_BLOCK_LEGS: {
   ...
   if (u_see)
    g->add_msg(P_("%s block with %s %s.", "%s blocks with %s %s.", is_npc()), You.c_str(),
...
// TODO: Cause weapon damage
  ...
  case TEC_DEF_THROW:
   if (u_see)
    g->add_msg(P_("%s throw %s!", "%s throws %s!", is_npc()), You.c_str(),
               target.c_str());
   bash_dam = 0;
   cut_dam  = 0;
   stab_dam = 0;
...
// Re-roll damage, without our weapon
   ...
   if (u_see)
    g->add_msg(P_("%s disarm %s!", "%s disarms %s!", is_npc()), You.c_str(),
                                  target.c_str());
   break;
 ...
 if (can_poison && has_trait(PF_POISONOUS)) {
  if (z != NULL) {
   ...
   if (!is_npc() && !p->has_disease(DI_POISON))
    g->add_msg(_("You poison %s!"), p->name.c_str());
                                   ...
                                   int &bash_dam, int &cut_dam, int &stab_dam)
{
 ...
 std::string You = (is_u ? _("You") : name);
 std::string Your = (is_u ? _("Your") : i18n::format(_("%s's"), name.c_str()));
 std::string your = (is_u ? _("your") : i18n::format(_("%s's"), name.c_str()));
 std::string target = (mon ? i18n::format(_("the %s"), z->name().c_str()) :
                       (p->is_npc() ? p->name : _("you")));
 std::string target_possessive = (mon ? i18n::format(_("the %s's"), z->name().c_str()) :
                                  (p->is_npc() ? i18n::format(_("%s's"), p->name.c_str()) : your));
...
// Stabbing effects
 ...
 if (stab_moves >= 150) {
  if (can_see)
   g->add_msg(P_("%s force the %s to the ground!", "%s forces the %s to the ground!", !is_u), You.c_str(),
              target.c_str());
  if (mon) {
...
// Bonus attacks!
 ...
 if (shock_them) {
  ...
  if (mon) {
   ...
   z->moves -= shock * 180;
   if (can_see)
    g->add_msg(P_("%s shock %s!", "%s shocks %s!", !is_u), You.c_str(),
               target.c_str());
  } else {
 ...
 if (drain_them) {
  ...
  if (can_see)
   g->add_msg(P_("%s drain %s body heat!", "%s drains %s body heat!", !is_u), You.c_str(),
...
// Getting your weapon stuck
 ...
 if (!unarmed_attack() && cutting_penalty > dice(str_cur * 2, 20)) {
  if (is_u)
   g->add_msg(_("Your %s gets stuck in %s, pulling it out of your hands!"),
  ...
  if (cutting_penalty >= 50 && is_u)
   g->add_msg(_("Your %s gets stuck in %s, but you yank it free."),
              weapon.tname().c_str(), target.c_str());
   ...
   if (has_disease(DI_VIPER_COMBO)) {
    if (disease_intensity(DI_VIPER_COMBO) == 1) {
     ...
     if (hp_cur[hp_arm_l] >= hp_max[hp_arm_l] * .75 &&
      ...
      if (is_u)
       g->add_msg(_("Viper STRIKE!"));
      bash_dam *= 3;
     } else if (is_u)
      g->add_msg(_("Your injured arms prevent a viper strike!"));
     rem_disease(DI_VIPER_COMBO);
    }
   } else if (crit) {
    if (is_u)
     g->add_msg(_("Tail whip!  Viper Combo Intiated!"));
    bash_dam += 5;
...
std::vector<special_attack> player::mutation_attacks(monster *z, player *p)
{
 ...
 if (has_trait(PF_FANGS) && !wearing_something_on(bp_mouth) &&
  ...
  special_attack tmp;
  text << i18n::format(P_("%s sink %s fangs into %s!", "%s sinks %s fangs into %s!", !is_u),
                 You.c_str(), your.c_str(), target.c_str());
  tmp.text = text.str();
  tmp.stab = 20;
  ret.push_back(tmp);
 }
 ...
 if (has_trait(PF_MANDIBLES) && one_in(22 - dex_cur - skillLevel("unarmed"))) {
  special_attack tmp;
  text << i18n::format(P_("%s slice %s with %s mandibles!", "%s slices %s with %s mandibles!", !is_u),
                 You.c_str(), target.c_str(), your.c_str());
  tmp.text = text.str();
  tmp.cut = 12;
  ret.push_back(tmp);
 }
 ...
 if (has_trait(PF_BEAK) && one_in(15 - dex_cur - skillLevel("unarmed"))) {
  special_attack tmp;
  text << i18n::format(P_("%s peck %s!", "%s pecks %s!", !is_u),
                 You.c_str(), target.c_str());
  tmp.text = text.str();
  tmp.stab = 15;
  ret.push_back(tmp);
 }
 ...
 if (has_trait(PF_HOOVES) && one_in(25 - dex_cur - 2 * skillLevel("unarmed"))) {
  special_attack tmp;
  text << i18n::format(P_("%s kick %s with %s hooves!", "%s kicks %s with %s hooves!", !is_u),
                 You.c_str(), target.c_str(), your.c_str());
  tmp.text = text.str();
  tmp.bash = str_cur * 3;
  if (tmp.bash > 40)
 ...
 if (has_trait(PF_HORNS) && one_in(20 - dex_cur - skillLevel("unarmed"))) {
  special_attack tmp;
  text << i18n::format(P_("%s headbutt %s with %s horns!", "%s headbutts %s with %s horns!", !is_u),
                 You.c_str(), target.c_str(), your.c_str());
  tmp.text = text.str();
  tmp.bash = 3;
  tmp.stab = 3;
 ...
 if (has_trait(PF_HORNS_CURLED) && one_in(20 - dex_cur - skillLevel("unarmed"))) {
  special_attack tmp;
  text << i18n::format(P_("%s headbutt %s with %s curled horns!", "%s headbutts %s with %s curled horns!", !is_u),
                 You.c_str(), target.c_str(), your.c_str());
  tmp.text = text.str();
  tmp.bash = 14;
  ret.push_back(tmp);
 ...
 if (has_trait(PF_HORNS_POINTED) && one_in(22 - dex_cur - skillLevel("unarmed"))){
  special_attack tmp;
  text << i18n::format(P_("%s stab %s with %s pointed horns!", "%s stabs %s with %s pointed horns!", !is_u),
                 You.c_str(), target.c_str(), your.c_str());
  tmp.text = text.str();
  tmp.stab = 24;
  ret.push_back(tmp);
 ...
 if (has_trait(PF_ANTLERS) && one_in(20 - dex_cur - skillLevel("unarmed"))) {
  special_attack tmp;
  text << i18n::format(P_("%s butt %s with %s antlers!", "%s butts %s with %s antlers!", !is_u),
                 You.c_str(), target.c_str(), your.c_str());
  tmp.text = text.str();
  tmp.bash = 4;
  ret.push_back(tmp);
 ...
 if (has_trait(PF_TAIL_STING) && one_in(3) && one_in(10 - dex_cur)) {
  special_attack tmp;
  text << i18n::format(P_("%s sting %s with %s tail!", "%s stings %s with %s tail!", !is_u),
                 You.c_str(), target.c_str(), your.c_str());
  tmp.text = text.str();
  tmp.stab = 20;
  ret.push_back(tmp);
 ...
 if (has_trait(PF_TAIL_CLUB) && one_in(3) && one_in(10 - dex_cur)) {
  special_attack tmp;
  text << i18n::format(P_("%s hit %s with %s tail!", "%s hits %s with %s tail!", !is_u),
                 You.c_str(), target.c_str(), your.c_str());
  tmp.text = text.str();
  tmp.bash = 18;
  ret.push_back(tmp);
 ...
 if (has_trait(PF_ARM_TENTACLES) || has_trait(PF_ARM_TENTACLES_4) ||
  ...
  for (int i = 0; i < num_attacks; i++) {
   if (one_in(18 - dex_cur - skillLevel("unarmed"))) {
    special_attack tmp;
    text.str("");
    text << i18n::format(P_("%s slap %s with %s tentacle!", "%s slaps %s with %s tentacle!", !is_u),
                   You.c_str(), target.c_str(), your.c_str());
    tmp.text = text.str();
    tmp.bash = str_cur / 2;
    ret.push_back(tmp);
                       ...
                       int bash_dam, int cut_dam, int stab_dam)
{
 if (tech != TEC_NULL && p.weapon.is_style() &&
  ...
  return i18n::format(_("%s's"), p.weapon.style_data(tech).name.c_str());
 ...
 switch (tech) {
  ...
  case TEC_SWEEP:
   ret << i18n::format(P_("sweep %s %s at", "sweeps %s %s at", p.is_npc()), your.c_str(), p.weapon.tname().c_str());
   break;

  case TEC_PRECISE:
   ret << i18n::format(P_("jab %s %s at", "jabs %s %s at", p.is_npc()), your.c_str(), p.weapon.tname().c_str());
   break;

  case TEC_BRUTAL:
   ret << i18n::format(P_("slam %s %s against", "slams %s %s against", p.is_npc()), your.c_str(), p.weapon.tname().c_str());
   break;

  case TEC_GRAB:
   ret << i18n::format(P_("wrap %s %s around", "wraps %s %s around", p.is_npc()), your.c_str(), p.weapon.tname().c_str());
   break;

  case TEC_WIDE:
   ret << i18n::format(P_("swing %s %s wide at", "swings %s %s wide at", p.is_npc()), your.c_str(), p.weapon.tname().c_str());
   break;

  case TEC_THROW:
   ret << i18n::format(P_("use %s %s to toss", "uses %s %s to toss", p.is_npc()), your.c_str(), p.weapon.tname().c_str());
   break;

  default: // No tech, so check our damage levels
   if (bash_dam >= cut_dam && bash_dam >= stab_dam) {
    if (bash_dam >= 30)
     return P_("clobber", "clobbers", p.is_npc());
    if (bash_dam >= 20)
     return P_("batter", "batters", p.is_npc());
    if (bash_dam >= 10)
     return P_("whack", "whacks", p.is_npc());
    return P_("hit", "hits", p.is_npc());
   }
   if (cut_dam >= stab_dam) {
    if (cut_dam >= 30)
     return P_("hack", "hacks", p.is_npc());
    if (cut_dam >= 20)
     return P_("slice", "slices", p.is_npc());
    if (cut_dam >= 10)
     return P_("cut", "cuts", p.is_npc());
    return P_("nick", "nicks", p.is_npc());
   }
// Only stab damage is left
   if (stab_dam >= 30)
    return P_("impale", "impales", p.is_npc());
   if (stab_dam >= 20)
    return P_("pierce", "pierces", p.is_npc());
   if (stab_dam >= 10)
    return P_("stab", "stabs", p.is_npc());
   return P_("poke", "pokes", p.is_npc());
 } // switch (tech)
...
{
 if (dam <= 0)
  g->add_msg(P_("%s %s %s but do no damage.", "%s %s %s but does no damage.", subject == _("You")), subject.c_str(), verb.c_str(),
             target.c_str());
 else
  g->add_msg(_("%s%s %s %s for %d damage."), (crit ? _("Critical! ") : ""),
