...
void iuse::bandage(game *g, player *p, item *it, bool t)
{
    ...
    if (p->is_npc()) { // NPCs heal whichever has sustained the most damage
        ...
        mvwprintz(hp_window, 1, 1, c_ltred,  _("Use Bandage:"));
        ...
        mvwprintz(hp_window, 8, 1, c_ltgray, _("7: Stop Bleeding"));
...
void iuse::firstaid(game *g, player *p, item *it, bool t)
{
    ...
    if (p->is_npc()) { // NPCs heal whichever has sustained the most damage
        ...
        wborder(hp_window, LINE_XOXO, LINE_XOXO, LINE_OXOX, LINE_OXOX,
                           LINE_OXXO, LINE_OOXX, LINE_XXOO, LINE_XOOX );
        mvwprintz(hp_window, 1, 1, c_ltred,  _("Use First Aid:"));
        mvwprintz(hp_window, 2, 1, c_ltgray, _("1: Head"));
        mvwprintz(hp_window, 3, 1, c_ltgray, _("2: Torso"));
        mvwprintz(hp_window, 4, 1, c_ltgray, _("3: Left Arm"));
        mvwprintz(hp_window, 5, 1, c_ltgray, _("4: Right Arm"));
        mvwprintz(hp_window, 6, 1, c_ltgray, _("5: Left Leg"));
        mvwprintz(hp_window, 7, 1, c_ltgray, _("6: Right Leg"));
        mvwprintz(hp_window, 8, 1, c_ltgray, _("7: Clean Wound"));
        mvwprintz(hp_window, 9, 1, c_ltgray, _("8: Exit"));
        nc_color color;
        std::string asterisks = "";
        int current_hp;
...
void iuse::dogfood(game *g, player *p, item *it, bool t)
{
 ...
 if (mon_dex != -1) {
  if (g->z[mon_dex].type->id == mon_dog) {
   ...
   g->add_msg_if_player(p,_("The %s seems quit unimpressed!"),g->z[mon_dex].type->name.c_str());
 } else
...
void iuse::sew(game *g, player *p, item *it, bool t)
{
 ...
 if (!fix->made_of(COTTON) && !fix->made_of(WOOL)) {
  g->add_msg_if_player(p,_("Your %s is not made of cotton or wool."), fix->tname().c_str());
  it->charges++;
  return;
 }
 if (fix->damage < 0) {
...
void iuse::scissors(game *g, player *p, item *it, bool t)
{
 ...
 if (cut->type->id == itm_rag) {
  g->add_msg_if_player(p,_("There's no point in cutting a rag."));
  return;
 }
 ...
 if (count <= 0) {
  g->add_msg_if_player(p,_("You clumsily cut the %s into useless ribbons."),
             cut->tname().c_str());
  p->i_rem(ch);
  return;
 }
 g->add_msg_if_player(p,P_("You slice the %s into %d rag.", "You slice the %s into %d rags.", count > 1),
 ...
 if (count <= 0) {
  g->add_msg_if_player(p,_("You clumsily cut the %s into useless scraps."),
             cut->tname().c_str());
  p->i_rem(ch);
  return;
 }
 g->add_msg_if_player(p,P_("You slice the %s into %d piece of leather.", "You slice the %s into %d pieces of leather.", count > 1),
            cut->tname().c_str(), count);
...
void iuse::hammer(game *g, player *p, item *it, bool t)
{
 ...
 case t_door_boarded:
  ...
  break;
 default:
  g->add_msg_if_player(p,_("Hammers can only remove boards from windows and doors."));
...
//       > Call another player
 ...
 mvwprintz(w, 2, 1, c_white, _("2: Call Acquaintance..."));
...
void iuse::picklock(game *g, player *p, item *it, bool t)
{
 int dirx, diry;
 g->draw();
 mvprintw(0, 0, _("Pick which lock?"));
 get_direction(g, dirx, diry, input());
 if (dirx == -2) {
  g->add_msg_if_player(p,_("Invalid direction."));
  return;
 ...
 if (npcdex != -1) {
  ...
  g->add_msg_if_player(p, _("pick your nose, but you can't pick"));
  g->add_msg_if_player(p, _("your friend's nose"));
  return;
 }
 ...
 if (dice(4, 6) < dice(2, p->skillLevel("mechanics")) + dice(2, p->dex_cur) - it->damage / 2) {
  p->practice("mechanics", 1);
  g->add_msg_if_player(p,_("With a satisfying click, the lock on the %s opens."), door_name);
  g->m.ter(dirx, diry) = new_type;
 } else if (dice(4, 4) < dice(2, p->skillLevel("mechanics")) +
                         dice(2, p->dex_cur) - it->damage / 2 && it->damage < 100) {
  it->damage++;

  std::string sStatus = _("damage");
  if (it->damage >= 5) {
   sStatus = _("destroy");
   it->invlet = 0; // no copy to inventory in player.cpp:4472 ->
  }

  g->add_msg_if_player(p,(_("The lock stumps your efforts to pick it, and you ") + sStatus + _(" your tool.")).c_str());
 } else {
 ...
 if ( type == t_door_locked_alarm &&
      ...
      dice(2, p->dex_cur) - it->damage / 2 && it->damage < 100) {
  g->sound(p->posx, p->posy, 30, _("An alarm sounds!"));
...
if (dirx == 0 && diry == 0) {
 ...
 if (type == t_door_c || type == t_door_locked || type == t_door_locked_alarm) {
   door_name = _("door");
   action_name = _("pry open");
   new_type = t_door_o;
   noisy = true;
   difficulty = 6;
 } else if (type == t_manhole_cover) {
   door_name = _("manhole cover");
   action_name = _("lift");
   new_type = t_manhole;
   noisy = false;
   difficulty = 12;
 ...
 if (dice(4, difficulty) < dice(2, p->skillLevel("mechanics")) + dice(2, p->str_cur)) {
  p->practice("mechanics", 1);
  g->add_msg_if_player(p,_("You %s the %s."), action_name, door_name);
  g->m.ter(dirx, diry) = new_type;
  if (noisy)
   g->sound(dirx, diry, 8, _("crunch!"));
  if ( type == t_door_locked_alarm ) {
   g->sound(p->posx, p->posy, 30, _("An alarm sounds!"));
   if (!g->event_queued(EVENT_WANTED)) {
    g->add_event(EVENT_WANTED, int(g->turn) + 300, 0, g->levx, g->levy);
  ...
  g->add_msg_if_player(p,_("You pry, but cannot %s the %s."), action_name, door_name);
 }
}
...
void iuse::firecracker_pack(game *g, player *p, item *it, bool t)
{
 ...
 mvwprintz(w, 3, 5, c_ltred, _("I"));
 mvwprintz(w, 3, 6, c_white, _("ncrease"));
 mvwprintz(w, 3, 14, c_ltred, _("D"));
 mvwprintz(w, 3, 15, c_white, _("ecrease"));
 mvwprintz(w, 3, 23, c_ltred, _("A"));
 mvwprintz(w, 3, 24, c_white, _("ccept"));
 mvwprintz(w, 3, 30, c_ltred, _("C"));
 mvwprintz(w, 3, 31, c_white, _("ancel"));
 wrefresh(w);
 bool close = false;
 int charges = 1;
 char ch = getch();
...
{
 int ch = menu(
 _("Using knife:"), _("Cut up fabric"), _("Carve wood"), _("Cancel"), NULL);
 switch (ch) {
  if (ch == 3)
  break;
...
if (cut->made_of(COTTON)) {
 ...
 g->add_msg(P_("You slice the %s into %d rag.", "You slice the %s into %d rags.", count > 1),
            cut->tname().c_str(), count);
 item rag(g->itypes[itm_rag], int(g->turn), g->nextinv);
 p->i_rem(ch);
 bool drop = false;
...
char ch = g->inv(_("Chop up what?"));
 ...
 g->add_msg(_("You carve several skewers from the wood."), cut->tname().c_str());
 int count = 8;
...
} else { g->add_msg(_("You can't carve that up!"));
  }
 }
break;
 }
...
void iuse::lumber(game *g, player *p, item *it, bool t)
{
 ...
 for (int i = 0; i < scraps; i++) {
  ...
  return;
  } else { g->add_msg(_("You can't cut that up!"));
 } return;
}
...
void iuse::hacksaw(game *g, player *p, item *it, bool t)
{
 int dirx, diry;
 g->draw();
 mvprintw(0, 0, _("Cut up metal where?"));
 get_direction(g, dirx, diry, input());
 if (dirx == -2) {
...
void iuse::tent(game *g, player *p, item *it, bool t)
{
 int dirx, diry;
 g->draw();
 mvprintw(0, 0, _("Put up tent where?"));
 get_direction(g, dirx, diry, input());
 ...
 for (int i = -1; i <= 1; i++)
  for (int j = -1; j <= 1; j++)
   if (!g->m.has_flag(tentable, posx + i, posy + j)) {
    g->add_msg(_("You need a 3x3 diggable space to place a tent"));
    return;
   }
 for (int i = -1; i <= 1; i++)
  for (int j = -1; j <= 1; j++)
...
void iuse::shelter(game *g, player *p, item *it, bool t)
{
 int dirx, diry;
 g->draw();
 mvprintw(0, 0, _("Put up shelter where?"));
 get_direction(g, dirx, diry, input());
 if (dirx == -2 || (dirx == 0 && diry == 0)) {
  g->add_msg_if_player(p,_("Invalid direction."));
  return;
 ...
 for (int i = -1; i <= 1; i++)
  for (int j = -1; j <= 1; j++)
   if (!g->m.has_flag(tentable, posx + i, posy + j)) {
    g->add_msg(_("You need a 3x3 diggable space to place a shelter"));
    return;
   }
 for (int i = -1; i <= 1; i++)
  for (int j = -1; j <= 1; j++)
...
if (dirx == 0 && diry == 0) {
  ...
  g->add_msg_if_player(p, _("and arteries in your body. Oh wait,"));
  g->add_msg_if_player(p, _("Never mind."));
  return;
}
 dirx += p->posx;
...
void iuse::rag(game *g, player *p, item *it, bool t)
{
 if (p->has_disease(DI_BLEED)){
  if (one_in(2)){
   ...
   g->add_msg_if_player(p,_("You couldnt stop the bleeding."));
  }
  p->use_charges(itm_rag, 1);
  it->make(g->itypes[itm_rag_bloody]);
 } else {
  g->add_msg_if_player(p,_("Nothing to use the rag for."));
 }
...
{
 std::string message = string_input_popup(_("Spray what?"));
 if(g->m.add_graffiti(g, p->posx, p->posy, message))
  g->add_msg(_("You spray a message on the ground."));
 else
  g->add_msg(_("You fail to spray a message here."));
}
